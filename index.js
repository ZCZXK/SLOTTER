...
async function executeCommand({ commandName, args, member, guild, channel, author, reply, deleteMsg }) {
  ...
  if (commandName === 'renameslot') {
    ...
    return reply({ embeds: [successEmbed('Slot Renamed', `Channel updated to \`〢⤷slot-${cleanName}\``)] });
  }
  ...
  if (commandName === 'stats') {
    ...
    return reply({ embeds: [embed] }, false); // Keep stats persistent
  }
  ...
  if (commandName === 'vouches') {
    ...
    return reply({ embeds: [new EmbedBuilder().setTitle(`📋 Vouches — ${target.username}`).setDescription(list).setFooter({ text: `Total Vouches: ${total}` }).setColor(0x5865F2)], false); // Keep list persistent
  }
}
...
client.on('messageCreate', async (message) => {
  ...
  // Auto-deleting message handler for prefix commands
  ...
  const deleteMsg = async () => message.deletable && message.delete().catch(() => {});
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    const reply = async (payload) => {
      payload.ephemeral = true; // Auto-hides after viewing (native Discord standard)
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId === 'mdl_claim_slot') {
      ...
      // Fetch Guild Config for custom Category placement
