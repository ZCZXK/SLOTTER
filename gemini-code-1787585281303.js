...
async function executeCommand({ commandName, args, member, guild, channel, author, reply, deleteMsg }) {
  ...
  if (commandName === 'setup') {
    ...
    return reply({ embeds: [successEmbed('Panels Deployed', 'VIP Slot and Ping panels successfully loaded into channel.')] });
  }
...
client.on('messageCreate', async (message) => {
  ...
  const reply = async (payload) => message.channel.send(payload);
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isChatInputCommand()) {
    ...
    args.targetChannel = interaction.options.getChannel('channel');
